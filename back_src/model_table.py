...
class Users(Base):
    ...
    following_relations: Mapped[List["Friends"]] = relationship(
        "Friends",
        foreign_keys="[Friends.follower_id]",
        back_populates="follower"
    )
    follower_relations: Mapped[List["Friends"]] = relationship(
        "Friends",
        foreign_keys="[Friends.following_id]",
...
class Friends(Base):
    ...
    follower_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), primary_key=True, nullable=False)
    following_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), primary_key=True, nullable=False)
...
class Messages(Base):
    ...
    content: Mapped[str] = mapped_column(nullable=False)
    sender_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), primary_key=True, nullable=False)
    recipient_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), primary_key=True, nullable=False)
    ...
    recipient: Mapped["Users"] = relationship(
        ...
        back_populates="received_message"
    )